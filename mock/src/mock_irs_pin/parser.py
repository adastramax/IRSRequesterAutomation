"""Input parsing and validation for the v7 mock IRS PIN workflow."""
...
REQUIRED_FIELDS = ("bod", "last_name", "first_name", "seid", "site_name")
...
def parse_input_dataframe(dataframe: pd.DataFrame) -> list[ParsedRow]:
    ...
    dataframe = dataframe.rename(columns=mapped_headers)
...
def parse_input_file(path: str | Path) -> list[ParsedRow]:
    ...
    return parse_input_dataframe(load_input_frame(input_path))
