"""FastAPI wrapper exposing the processor through a single main endpoint."""
...
app = FastAPI(title="IRS PIN Tool")
...
class InputRowRequest(BaseModel):
    ...
    bod: str = Field(default="", alias="BOD", description="IRS BOD/customer code.")
    customer_name: str = Field(default="", alias="Customer Name", description="Explicit customer/account name when BOD is not supplied.")
...
class ProcessRowsRequest(BaseModel):
    ...
    created_by: str = Field(default="IRS PIN Operator", description="Audit user name for commit calls.")
    write_output: bool = Field(default=False, description="Write batch output file when true.")
    debug: bool = Field(default=False, description="Return raw debug shape when true on commit.")
...
def _build_result_message(row_result: dict[str, Any]) -> str:
    if row_result.get("status") == "Manual Intervention Required":
        modify = row_result.get("modify_function") or {}
        deactivate_step = modify.get("deactivate_step") or {}
        create_step = modify.get("create_step") or {}
        if deactivate_step.get("success"):
            create_message = str(create_step.get("message") or "").strip()
            if create_message:
                return f"Old site was already deactivated, but the new-site create failed: {create_message}"
            return "Old site was already deactivated, but the new-site create failed. Manual intervention is required."
        ...
        results.append(
            {
                ...
                "corrected_data": {
                    ...
                    "modify_function": row_result.get("modify_function") or {},
                ...
                "deactivate_step": ((row_result.get("modify_function") or {}).get("deactivate_step") or {}),
                "create_step": ((row_result.get("modify_function") or {}).get("create_step") or {}),
                "result": {
...
def _run_review_request(rows: list[dict[str, Any]], *, debug: bool) -> dict[str, Any]:
    return _review_rows([InputRowRequest.model_validate(row) for row in rows], debug=debug)
...
def _backfill_missing_review_pin_context(customer_name: str, pin_context: dict[str, Any], teid_resolution: dict[str, Any] | None, customer_context: dict[str, Any]) -> dict[str, Any]:
    if is_missing_fk_value(pin_context.get("fK_Customer")):
        fallback_teid = normalize_teid((teid_resolution or {}).get("currentMaxTeid"))
        if fallback_teid:
            try:
                fallback_context = app.state.client.get_pin_context(customer_name, fallback_teid)
            ...
            if customer_context is None:
                ...
                if row.contact_status == "Modify-Function Change":
                    ...
                    if destination_teid and row.employee_id:
                        first_choice_pin = f"{destination_teid}{str(row.employee_id).strip()}"
                        notes.append("Modify-Function Change will first try preserving the existing Employee ID under the new TEID.")
                    ...
                    modify_function = {
                        ...
                        "fallback_rule": "Retry once with target TEID maxPinCode + 1 if Connect rejects the preserved employee-id PIN as duplicate.",
                    }
                    corrected_site_name = destination_site_name
                    resolved_site_id = destination_teid
                    trace_logs.append(
                        {
                            "stage": "modify_function_review",
                            "message": "Prepared composite review for modify-function change",
                            "row_number": row.row_number,
                            "details": modify_function,
                        }
                    )
                    if destination_teid_resolution is not None:
                        trace_logs.append(
                            {
                                "stage": "teid_resolution",
                                "message": "Resolved destination TEID state for review",
                                "row_number": row.row_number,
                                "details": destination_teid_resolution,
                            }
                        )
                    summary["Reviewed"] += 1
                    results.append(
...
@app.post("/process/review-file", include_in_schema=False)
async def process_review_file(
    file: UploadFile = File(...),
    debug: bool = Form(default=True),
) -> dict[str, Any]:
    try:
        parsed_rows = parse_input_bytes(file.filename, await file.read())
        request_rows = [InputRowRequest.model_validate(
            {
                "BOD": row.bod,
